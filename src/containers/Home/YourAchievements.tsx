...
import { Card, Box, useMediaQuery, useTheme, Checkbox } from "@mui/material";
...
const YourAchievements = () => {
  ...
  const getInvitationLink = (role: string, status: boolean) => {
    if (status) {
      ...
      Axios.get(
        `https://www.cryptolegions.link/api/get-roles/check/${role}/${account}`
      )
        .then((res) => {
  ...
  return (
    ...
    >
      <Box sx={{ p: 4, justifyContent: "center" }} className="legionFontColor">
        <Typography
          sx={{
            p: 1,
            fontSize: 18,
        ...
        <Typography
          sx={{ p: 1 }}
          className={classnames({ achievementColor: warriorMaster })}
          onClick={() => getInvitationLink("warrior_master", warriorMaster)}
        >
        ...
        <Typography
          sx={{ p: 1 }}
          className={classnames({ achievementColor: beastMaster })}
          onClick={() => getInvitationLink("beast_master", beastMaster)}
        >
        ...
        <Typography
          sx={{ p: 1 }}
          className={classnames({ achievementColor: legionMaster })}
          onClick={() => getInvitationLink("legion_master", legionMaster)}
        >
        ...
        <Typography
          sx={{ p: 1 }}
          className={classnames({ achievementColor: monsterConqueror })}
          onClick={() =>
            getInvitationLink("monster_conqueror", monsterConqueror)
          }
        >
        ...
        <Typography
          sx={{ p: 1 }}
          className={classnames({ achievementColor: kingOfNicah })}
          onClick={() => getInvitationLink("king_of_nicah", kingOfNicah)}
        >
